...
class Map:
        ...
        for j in range(4, len(self.my_map[0]), 10):  # Вертикальные коридоры
            ...
            for i in range(len(self.my_map)):
                if self.my_map[i][j].type == 'wall':
                    ...
                    if not w_count % 2:
                        cor_flag = True
                        # self.my_map[i + 1][j] = Tile(Tile(f, j * i, 'door'))
                        # Здесь можно дверь добавить, если понадобится
                    else:
                        cor_flag = False
                        if w_count > 1:
        ...
        for i in range(4, len(self.my_map), 10):  # Горизонтальные коридоры
            ...
            for j in range(len(self.my_map[i])):
                if self.my_map[i][j].type == 'wall':
                    ...
                    if not w_count % 2:
                        cor_flag = True
                        # self.my_map[i + 1][j] = Tile(
                        # Tile(f, j * i, 'door')) Здесь можно дверь добавить, если понадобится
                    else:
                        cor_flag = False
                        if w_count > 1:
    ...
    def place(self):
        ...
        #  print(empty_floors[-1], empty_floors[-1].get_pos()[0],
        #  empty_floors[-5].get_pos()[1], self.my_map[empty_floors[-1].get_pos()[0]][empty_floors[-5].get_pos()[1]])

        #  print(empty_floors[-1].get_pos() == self.my_map[empty_floors[-1].get_pos()[0]]
        #  [empty_floors[-1].get_pos()[1]].get_pos())
