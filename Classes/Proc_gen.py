...
class Map:
    ...
    def create_map(self):
        ...
        for j in range(4, len(self.my_map[0]), 10):  # Вертикальные коридоры
            ...
            for iuio in range(len(self.my_map)):
                if self.my_map[iuio][j].type == 'wall':
                    ...
                    if not w_count % 2:
                        cor_flag = True
                        #  self.my_map[iuio + 1][j] = Tile(Tile(f, j * iuio, 'door')) Здесь можно дверь добавить,
                        #  если понадобится
                    else:
                        cor_flag = False
                        if w_count > 1:
        ...
        for iuio in range(4, len(self.my_map), 10):  # Горизонтальные коридоры
            w_count = 0
            cor_flag = False
            for j in range(len(self.my_map[iuio])):
                if self.my_map[iuio][j].type == 'wall':
                    ...
                    if not w_count % 2:
                        cor_flag = True
                        #  self.my_map[iuio + 1][j] = Tile(Tile(f, j * iuio, 'door')) Здесь можно дверь добавить, если
                        #  понадобится
                    else:
                        cor_flag = False
                        if w_count > 1:
